...
const sendWithdrawalRequestEmail = async ({ from, amount, method, address }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER || "no-reply@Ultradeplus.com",
      to: "support@Ultradeplus.com",
      ...
      html: `
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>Best regards,<br>Ultradeplus Team</p>
              <p style="font-size: 12px; color: #888;">© ${new Date().getFullYear()} Ultradeplus. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendWelcomeEmail = async ({ to, otp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: to,
      subject: "Verify Your Ultradeplus Account",
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1.0" />
          <title>Ultradeplus Verification</title>
          <style>
            body {
        ...
        <body>
          <div class="container">
            <div class="header">
              <img src="https://Ultradeplus.com/logo.png" alt="Ultradeplus Logo" />
              <h1>Welcome to Ultradeplus</h1>
            </div>
            <div class="content">
              <h2>Verify Your Email</h2>
              <p>Hello,</p>
              <p>Thank you for creating an account with <strong>Ultradeplus</strong>. To protect your account, please use the verification code below to complete your registration:</p>

              <div class="otp-box">
                ${otp}
              </div>

              <p>This code will expire in <strong>5 minutes</strong>. For your security, never share this code with anyone—even a Ultradeplus representative.</p>

              <div class="note">
                If you didn’t sign up for a Ultradeplus account, please ignore this email or contact our support team immediately.
              </div>
            </div>
            <div class="footer">
              <p>Need help? Contact our <a href="mailto:support@Ultradeplus.com">support team</a>.</p>
              <p>&copy; ${new Date().getFullYear()} Ultradeplus. All rights reserved.</p>
            </div>
          </div>
        </body>
        </html>
...
const sendAdminWithdrawalNotification = async ({ userName, userEmail, amount, method, timestamp }) => {
  try {
    await resend.emails.send({
      from: process.env.EMAIL_USER,
      to: "support@Ultradeplus.com", // admin email from environment variable
